...
// Khởi tạo thư mục data và thư mục con notes
...
// Khởi tạo ghi chú mẫu trong hoc-tap.json
const sampleHocTap = getFilePath('hoc-tap');
if (!fs.existsSync(sampleHocTap)) {
    const initialHocTap = [
        {
            id: Date.now().toString(),
            title: "Kế hoạch bảo vệ đồ án",
            content: "1. Hoàn thiện Backend và Frontend.\n2. Chuẩn bị slide và luyện tập thuyết trình.",
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
        }
    ];
    fs.writeFileSync(sampleHocTap, JSON.stringify(initialHocTap, null, 2), 'utf8');
}
...
// Khởi tạo ghi chú mẫu trong private.json
if (!fs.existsSync(privateNotesFile)) {
    const defaultPrivateNotes = [
        {
            id: Date.now().toString(),
            title: "Mật khẩu & Tài khoản cá nhân",
            content: "Mật khẩu mặc định truy cập vùng kín là: 123 (Có thể đổi ở menu Cài đặt).",
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
        }
    ];
...
//4. API Sửa Ghi chú riêng tư
app.put('/api/private/notes/:id', (req, res) =>{
    try{
        let notes = JSON.parse(fs.readFileSync(privateNotesFile, 'utf-8'));
        const index =notes.findIndex(n => n.id === req.params.id);
        if (index !== -1) {
            notes[index].title = req.body.title;
            notes[index].content = req.body.content;
